...
fn build_request_log_sample(row: RequestLogAnalyticsRow) -> CodexReasoningAnalyticsSample {
    ...
    let request_kind =
        read_string(guard.and_then(|value| value.get("requestKind"))).unwrap_or_else(|| "turn".to_string());
    ...
    let output_tps = (duration_ms > 0).then(|| {
        round_metric((output_tokens as f64 * 1000.0) / duration_ms as f64, 4)
    });
    let reasoning_adjusted_tps = (duration_ms >= 250).then(|| {
        round_metric(
            ((reasoning_tokens.unwrap_or(0) + output_tokens) as f64 * 1000.0)
                / duration_ms as f64,
            4,
        )
    });
    ...
    CodexReasoningAnalyticsSample {
        ...
        internal_retry_remaining: read_i64(guard.and_then(|value| value.get("guardBudgetRemaining"))),
        final_action,
...
fn sample_from_value(value: &Value, source_name: &str, index: usize) -> Option<CodexReasoningAnalyticsSample> {
    ...
    Some(CodexReasoningAnalyticsSample {
        ...
        final_action: read_string(obj.get("final_action")).unwrap_or_else(|| "imported".to_string()),
...
fn extract_import_samples(root: &Value) -> Vec<Value> {
    ...
    for pointer in ["/samples", "/recent_samples", "/data/samples", "/summary/samples"] {
...
fn store_sample(conn: &rusqlite::Connection, sample: &CodexReasoningAnalyticsSample) -> AppResult<bool> {
...
fn top_reasoning_tokens(
    ...
    let mut rows = counts
        ...
        .collect::<Vec<_>>();
    rows.sort_by(|left, right| right.count.cmp(&left.count).then(left.value.cmp(&right.value)));
    rows.truncate(limit);
    rows
}
...
fn grouped_row(key: String, group: &[CodexReasoningAnalyticsSample], total: usize) -> CodexReasoningGroupedRow {
    ...
    CodexReasoningGroupedRow {
        ...
        final_answer_only_ratio: if count == 0 {
            ...
            round_metric(
                group.iter().filter(|sample| sample.final_answer_only).count() as f64 / count as f64,
                6,
            )
        },
        ...
        avg_duration_total_ms: round_metric(
            avg_metric(group, |sample| sample.duration_total_ms.map(|value| value as f64)),
            2,
        ),
    ...
    F: Fn(&CodexReasoningAnalyticsSample) -> String,
{
    ...
    for sample in samples {
        groups.entry(key_fn(sample)).or_default().push(sample.clone());
    }
    let mut rows = groups
        .into_iter()
...
fn output_tps_buckets(samples: &[CodexReasoningAnalyticsSample]) -> Vec<CodexReasoningOutputTpsBucket> {
    let mut buckets = vec![
        ("0-5".to_string(), 0),
        ("5-15".to_string(), 0),
        ("15-30".to_string(), 0),
...
fn candidate_patterns(samples: &[CodexReasoningAnalyticsSample]) -> Vec<CodexReasoningCandidatePattern> {
    let mut groups: HashMap<String, Vec<CodexReasoningAnalyticsSample>> = HashMap::new();
    for sample in samples {
        if let Some(tokens) = sample.reasoning_tokens {
    ...
    let mut rows = groups
        ...
        .map(|(pattern_key, group)| {
            ...
            let status = if group.iter().any(|sample| sample.blocked_by_gateway) {
                ...
                .iter()
                .all(|sample| sample.intercept_exempt_reason.as_deref() == Some("context_compaction"))
            {
        ...
        .map(|(value, group)| {
            ...
            CodexReasoningTokenRow {
                ...
                final_answer_only_ratio: round_metric(
                    group.iter().filter(|sample| sample.final_answer_only).count() as f64 / count,
                    6,
                ),
                commentary_observed_ratio: round_metric(
                    group.iter().filter(|sample| sample.commentary_observed).count() as f64 / count,
                    6,
                ),
                avg_duration_total_ms: round_metric(
                    avg_metric(&group, |sample| sample.duration_total_ms.map(|value| value as f64)),
...
fn snapshot_from_samples(
    ...
    let final_only = samples.iter().filter(|sample| sample.final_answer_only).count() as f64;
    let commentary = samples.iter().filter(|sample| sample.commentary_observed).count() as f64;
    ...
    .into_iter()
    .map(|row| {
        let (model_family, reasoning_effort) =
            row.key.split_once('|').unwrap_or((row.key.as_str(), "unknown"));
...
pub fn snapshot(
    ...
    Ok(snapshot_from_samples(samples, date_from, date_to, recent_limit))
}
    ...
    for sample in samples {
        lines.push(
            [
                ...
                csv_escape(sample.request_log_id.map(|v| v.to_string()).unwrap_or_default()),
                ...
                csv_escape(sample.reasoning_tokens.map(|v| v.to_string()).unwrap_or_default()),
                csv_escape(sample.output_tokens.map(|v| v.to_string()).unwrap_or_default()),
                csv_escape(sample.total_tokens.map(|v| v.to_string()).unwrap_or_default()),
                csv_escape(sample.duration_total_ms.map(|v| v.to_string()).unwrap_or_default()),
                csv_escape(sample.output_tps.map(|v| v.to_string()).unwrap_or_default()),
                csv_escape(sample.reasoning_adjusted_tps.map(|v| v.to_string()).unwrap_or_default()),
                ...
                csv_escape(sample.internal_retry_attempt_index.map(|v| v.to_string()).unwrap_or_default()),
                csv_escape(sample.internal_retry_remaining.map(|v| v.to_string()).unwrap_or_default()),
                csv_escape(&sample.final_action),
                csv_escape(sample.upstream_http_status.map(|v| v.to_string()).unwrap_or_default()),
                csv_escape(sample.client_http_status.map(|v| v.to_string()).unwrap_or_default()),
...
fn field_coverage(samples: &[CodexReasoningAnalyticsSample]) -> CodexReasoningFieldCoverage {
    ...
    CodexReasoningFieldCoverage {
        reasoning_tokens: ratio(samples.iter().filter(|sample| sample.reasoning_tokens.is_some()).count()),
        final_answer_only: ratio(samples.len()),
        commentary_observed: ratio(samples.len()),
        duration_total_ms: ratio(samples.iter().filter(|sample| sample.duration_total_ms.is_some()).count()),
        output_tokens: ratio(samples.iter().filter(|sample| sample.output_tokens.is_some() || sample.total_tokens.is_some()).count()),
        model_family: ratio(samples.iter().filter(|sample| sample.effective_local_model_family != "unknown").count()),
        reasoning_effort: ratio(samples.iter().filter(|sample| sample.request_reasoning_effort.is_some()).count()),
        status: ratio(samples.iter().filter(|sample| sample.client_http_status.is_some() || sample.upstream_http_status.is_some() || !sample.final_action.is_empty()).count()),
        retry_status: ratio(samples.iter().filter(|sample| sample.internal_retry_attempt_index.is_some() || sample.internal_retry_remaining.is_some()).count()),
    ...
    Ok(CodexReasoningAnalysisResult {
        ...
        candidate_summary: CodexReasoningCandidateSummary {
            ...
            high_time_normalization_deviation_count: candidate_samples
                ...
                .count() as i64,
            last_seen_at: candidate_samples.iter().map(|sample| sample.ts.clone()).max(),
        },
        baseline_comparison: CodexReasoningBaselineComparison {
            baseline_count,
            candidate_avg_time_normalization_deviation: round_metric(
                avg_metric(&candidate_samples, |sample| sample.time_normalization_deviation),
                6,
            ),
            baseline_avg_time_normalization_deviation: round_metric(
                avg_metric(&baseline_samples, |sample| sample.time_normalization_deviation),
                6,
            ),
